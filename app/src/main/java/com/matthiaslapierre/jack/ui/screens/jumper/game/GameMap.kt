...
import com.matthiaslapierre.jack.ui.screens.jumper.game.sprites.collectibles.CandySprite
import com.matthiaslapierre.jack.ui.screens.jumper.game.sprites.collectibles.PowerUpSprite
import com.matthiaslapierre.jack.ui.screens.jumper.game.sprites.obstacles.BatSprite
...
class GameMap(
    ...
    companion object {
        ...
        private const val CANDIES_MIN_INTERVAL_RATIO = 0.2f
        private const val CANDIES_MAX_INTERVAL_RATIO = 0.3f
        private const val JUMPING_PLATFORM_MIN_INTERVAL_RATIO = 0.2f
        private const val JUMPING_PLATFORM_MAX_INTERVAL_RATIO = 0.3f
        private const val BAT_MIN_INTERVAL_RATIO = 0.2f
        private const val BAT_MAX_INTERVAL_RATIO = 0.3f
    ...
    fun generate(): List<Sprite> {
        synchronized(this) {
            ...
            if (lastGeneratedSprite != null) {
                ...
                nextSpriteY = lastGeneratedSprite!!.y - getSpriteInterval(previousPattern)
            }
            while (nextSpriteY > -(screenHeight * 2f)) {
                ...
                nextSpriteY -= getSpriteInterval(pattern)
            }
    ...
    private fun getSpriteInterval(previousPattern: Int?): Float =
        when (previousPattern) {
            PATTERN_CANDIES_LINE, PATTERN_CANDIES_MATRIX -> Utils.getRandomFloat(
                screenWidth * CANDIES_MIN_INTERVAL_RATIO,
                screenWidth * CANDIES_MAX_INTERVAL_RATIO
            )
            PATTERN_BAT_STATIC, PATTERN_BAT_DYNAMIC -> Utils.getRandomFloat(
                screenWidth * BAT_MIN_INTERVAL_RATIO,
                screenWidth * BAT_MAX_INTERVAL_RATIO
            )
            else -> Utils.getRandomFloat(
                screenWidth * JUMPING_PLATFORM_MIN_INTERVAL_RATIO,
                screenWidth * JUMPING_PLATFORM_MAX_INTERVAL_RATIO
            )
        }
    ...
    private fun buildCandiesLineSprites(x: Float, y: Float): List<Sprite> {
        ...
        val capacity = Utils.getRandomInt(1, MAX_CANDIES_Y_CAPACITY)
        ...
        for (index in 0 until capacity) {
            ...
            val sprite = if (!powerUpGenerated
                && ((lastPowerUpSprite == null && y <= getNextPowerUpInterval())
    ...
    private fun buildCandiesMatrixSprites(x: Float, y: Float): List<Sprite> {
        val sprites = mutableListOf<Sprite>()
        val capacityX = Utils.getRandomInt(1, MAX_CANDIES_X_CAPACITY)
        val capacityY = Utils.getRandomInt(1, MAX_CANDIES_Y_CAPACITY)
        val outset = (screenWidth * CANDY_OUTSET_RATIO)
        val spaceX = (screenWidth * CANDY_SPACE_X_RATIO)
        val spaceY = (screenWidth * CANDY_SPACE_Y_RATIO)
    ...
    private fun getRandomPattern(y: Float): Int {
        ...
        return if ((pattern == PATTERN_BAT_STATIC || pattern == PATTERN_BAT_DYNAMIC)
            && ((lastObstacleSprite != null && abs(y - lastObstacleSprite!!.y) >= getNextObstacleInterval())
                    || (lastObstacleSprite == null || gameStates.elevation >= getNextObstacleInterval()))) {
